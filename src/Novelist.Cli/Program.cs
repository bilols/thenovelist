...
namespace Novelist.Cli
{
    internal static class Program
    {
        ...
        private static int Main(string[] args)
        {
            ...
            ILlmClient client = live
                ? CreateLiveClient(config) ?? new Stub()
                : new Stub();
            ...
            return (args[0].ToLowerInvariant(), args[1].ToLowerInvariant()) switch
            {
                ...
                ("outline", "draft-prologue")        => RunProEpi(args[2..], client, true),
                ("outline", "draft-epilogue")        => RunProEpi(args[2..], client, false),
        ...
        // ---------- NEW: prologue / epilogue producer ------------------------
        private static int RunProEpi(string[] a, ILlmClient llm, bool isPro)
        {
            if (!Try(a, "--outline", out var o))
                return Err("--outline required");
            if (!Try(a, "--output", out var outDir))
            ...
            new PrologueEpilogueBuilderService(llm)
                .BuildAsync(o, outDir, model, isPro).Wait();

            return Ok(isPro ? "Prologue drafted." : "Epilogue drafted.");
        }
        // ---------------------------------------------------------------------
        ...
        private static int Help()
        {
            ...
            AnsiConsole.WriteLine(" outline draft-prologue  --outline <file> --output <dir> [--model <id>] [-l] [-s]");
            AnsiConsole.WriteLine(" outline draft-epilogue  --outline <file> --output <dir> [--model <id>] [-l] [-s]");
