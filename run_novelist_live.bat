...
REM  Prerequisites:
REM    • 'dotnet user-secrets set OpenAI:Key "sk-..."' executed once in src\Novelist.Cli
REM    • 'dotnet' on PATH
REM ---------------------------------------------------------------------------
...
REM -------- pick the newest outline_*.json created in OUTDIR ------------------
for /f "delims=" %%f in ('dir /b /o:-d "%OUTDIR%\outline_*.json"') do (
    set "OUTLINE=%OUTDIR%\%%f"
    goto :have_outline
)

echo ERROR: No outline_*.json found in %OUTDIR%.
goto :error
